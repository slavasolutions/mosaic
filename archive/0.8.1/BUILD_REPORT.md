...
## Spec critic — independent review pass

A read-only spec-critic agent reviewed `TRUTHS.md`, `SPEC.md`, `ARCHITECTURE.md`, the JSON schema, all 13 MIPs, and a sample of conformance tests. Verdict: **REVISE** before tagging 0.8 final. Found 5 critical (interop-class) and 6 high (confusion-class) issues. **Fixed in commit `aa1c0b0`.**

Critical (C-class) — would cause two engines to disagree:
- C1: `mosaic.relative.invalid` had contradictory definitions in §5.5 vs §6.2. Reconciled.
- C2: Auto-redirect `/home → /` shape unspecified across index.redirects[] vs routes. Pinned MUST appear in both.
- C3: `routes` was specified as URL-keyed map in §7.1 but every test fixture used array-of-objects. Picked array-of-objects (matches tests); added per-record field table + invariants.
- C4: `§5.6` selector grammar disallowed uppercase in object keys, but DTCG camelCase is common. Widened to `[A-Za-z0-9_-]+`.
- C5: `redirects` singleton shape underspecified. Pinned `{ rules: RedirectRule[] }`.

High (H-class) — confusion for readers/authors:
- H1: `prose` section pattern used in §2.7 + every example but never normative. Promoted to §4.1 as a normative section type.
- H2: `README.md`/`LICENSE`/etc. inside `pages/` or `collections/` would become records. Reserved in §2.6.
- H3: Slug case in refs was ambiguous (ABNF lowercase-only, §2.5 "case-insensitive lookup"). Picked lowercase-only-in-refs; case-insensitivity is for collision detection on disk.
- H4: §3.3 trailing-slash rule was "MUST not + engines SHOULD normalize + strict tools MAY error" — three competing rules. Picked normalize-with-warning.
- H5: object-typed field with no `fields` was uncovered. Added free-form rule (mirror of §8.3.1 type-level escape hatch).
- H6: routes/pages/collections-records relationship unspecified. Added explicit invariants in §7.1.

Open follow-ups (M-class polish, L-class typos): see the critic's full report in commit `aa1c0b0` history.

## Astro starter blog migration test

Second migrator test: cloned `withastro/astro` `examples/blog`, ran the migrator non-interactively, validated.

Result: **0 structural, 0 drift, 2 warnings (asset orphans).** Migration produces a valid Mosaic site shape. But hidden in the output: **6 real migrator bugs** that don't fire as diagnostics but corrupt or skip content:

- **M4 (CRITICAL, FIXED in `aa1c0b0`)** — date truncation. `String(fm.pubDate).slice(0, 10)` silently corrupted non-ISO dates: Astro's `"Jul 08 2022"` (11 chars) → `"Jul 08 202"`. clear-ucc happened to use ISO dates so this never fired before. Fixed via `normalizeDate()` helper that parses via `Date()` then formats to YYYY-MM-DD.
- **M5** — `src/assets/` directory not scanned. Astro's `image()` helper resolves hero images from `src/assets/`, not `public/`. Migrator preserves the raw `"../../assets/blog-placeholder-3.jpg"` string in the `heroImage` field → broken after migration. Follow-up.
- **M6** — MDX bodies pass JSX through. `.mdx` files are emitted as `.md` with `<HeaderLink>` JSX and `import` lines intact. Renderer won't process; build fails downstream. Follow-up.
- **M7** — `$astro.source` hardcoded `src/pages/[...locale]/${rel}.astro`. Stale clear-ucc-specific path. Should derive from actual scanned path. Follow-up.
- **M8** — `src/consts.ts` (`SITE_TITLE`, `SITE_DESCRIPTION`) not parsed. Astro starter convention; migrator falls back to `package.json#name` (`@example/blog`) which isn't what the site is named. Follow-up.
- **M9** — Asset-typed fields don't reject literal relative paths. Validator currently allows `"../../assets/x.jpg"` in an asset-typed field. Could be a `--strict` flag. Follow-up.

The big finding: **migrators that pass the validator can still silently corrupt content.** The validator only checks spec rules. Source-format-specific bugs (date parsing, asset roots, MDX handling) need their own test suite per source format.

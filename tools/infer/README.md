...
1. Walks `pages/`, `collections/`, `globals/`.
...
3. For globals, each file becomes its own type.
4. Writes `mosaic.json` to the site root (or `--out`). Refuses to overwrite without `--overwrite`.
5. Adds `"_inferred": true` to the top of the file so future runs can tell hand-edited from generated.
...
- Custom types are not detected; everything becomes a fresh type named after its collection/global.
